...
def understandOurWay():                                                 # Функция определения способа ввода (файл / мышь / клавиатура)
    if(add == False):
        ...
        inputMethod.place(x = 300, y = 600)
        rad0.place(x = 300, y = 650)
        rad1.place(x = 300, y = 690)
        rad2.place(x = 300, y = 730)
        fb.place(x = 310, y = 780)
...
def tchCvs1():                                                          # Получение количества прямоугольников
    if(add == False):
        ...
        getPointsNum.delete(0, END)
        getPointsNum.place(x = 80, y = 650)
        startText.place(x = 110, y = 600)
        confirmBtnSP.place(x = 340, y = 744)
...
def confirmBtnSP_was_clicked():                                         # Приём количества точек и начало обработки
    ...
    if(str0.isdigit() == TRUE):
        if(int(str0) > 0):
            ...
            if(add == False):
                pointRemain.place(x = 310, y = 400)
            else:
                pointRemain.place(x = 310, y = 800)
...
def getCountOfPoints():                                                 # Отрисовываем строку ввода количесва точек, надпись и кнопку. Ждём её нажатия
    ...
    if(add == False):
        ...
        getPointsNum.place(x = 80, y = 750)
        startText.place(x = 110, y = 700)
        confirmBtn.place(x = 340, y = 744)
...
def getThePoints():                                                     # Функция сбора значений точек
    if(add == True):
        inputTextX1.place(x = 76, y = 490)                                  # Разместим 4 строки ввода
        inputTextY1.place(x = 76, y = 570)
        inputTextX2.place(x = 76, y = 650)
        inputTextY2.place(x = 76, y = 730)

        getPointX1.place(x = 80, y = 520)                                   # Разместим 4 текста
        getPointY1.place(x = 80, y = 600)
        getPointX2.place(x = 80, y = 680)
        getPointY2.place(x = 80, y = 760)

        confirmBtn2.place(x = 350, y = 625)                                 # И кнопку "Принять"

        fb2.place(x=450, y=850)

        randButton.place(x = 150, y = 950)                                  # А также кнопку получения рандомных значений
        rad10.place(x = 250, y = 820)                                       # И выбор, в каком диапазоне числа рандомить (до 10, до 50, до 100)
        rad11.place(x = 250, y = 860)
        rad12.place(x = 250, y = 900)
    else:
        ...
        fb2.place(x=510, y=600)

        randButton.place(x = 150, y = 850)                                  # А также кнопку получения рандомных значений
...
def fb2_was_clicked():
    ...
    if(add == False):
        getPointsNum.place(x = 80, y = 150)
        startText.place(x = 110, y = 100)
        fb3.place(x = 340, y = 144)
    else:
        getPointsNum.place(x = 80, y = 750)
        startText.place(x = 110, y = 700)
        fb3.place(x = 340, y = 744)

def fb3_was_clicked():
   
    ccc = int(countOfPoints / 4)
    ...
    for i in range (0, ccc):
        ...
        if(d == b):
            d = d - random.randint(1, 4)
        if (a != c and b != d):
            arrPoints.append([a, b, c, d])
...
def mainTask():                                                         # Перейдём к выполнению основного задания
    ...
    for i in range (0, len(arrPoints2)):
        ...
        for j in range(0, len(arrPoints2)):
            ...
            if(i != j):                                                 # Чтобы не сравнивать положения одного и того же элемента
...
#---------------------------------------------------------------------------------
                ...
                r = math.sqrt((ss2[0]-ss1[0])**2 + (ss2[1]-ss1[1])**2)  # По известной всем формуле подсчитаем расстояние между точками
    ...
    zanovo.place(x = 300, y = 100)
    save.place(x = 270, y = 220)
    db.place(x = 300, y = 350)
...
width = 20, bg='#B2EC5D', command = zanovo_was_clicked)

b1 = Button(text="Открыть файл", font='Arial 14',
width = 20, bg='#C018FF', command=opFl1)

db = Button(text="Добавить прямоугольники", font='Arial 14',
width = 20, bg='#228337', command=db_was_clicked)

fb = Button(text = 'Применить', font='Arial 14',
width = 10, bg='#B2EC5D', command = fb_was_clicked)

fb2 = Button(text = 'FULL RANDOM', font='Arial 14',
width = 30, bg='#D45A22', command = fb3_was_clicked)
fb3 = Button(text = 'Применить', font='Arial 14',
width = 20, bg='#525252', command = fb3_was_clicked)
...
understandOurWay()                                                      # начало программы
window.mainloop()                                                       # запускает цикл обработки событий. Без него ничего не заработает
